...
async function runContentStrategyValidation() {
  ...
  try {
    ...
    const { ContentCalendarGenerator, ContentCalendarUtils } = await import('./src/generators/content-calendar.js');
    const { InternalLinkingEngine, InternalLinkingUtils } = await import('./src/generators/internal-links.js');
    ...
    console.log(`✅ Policy compliance: ${Math.round(((linkingStrategy.metadata.totalOpportunities - linkingStrategy.metadata.policyViolations) / linkingStrategy.metadata.totalOpportunities) * 100)}%`);
    ...
    // 1. Content calendar with 12+ pieces over Q1
    const contentCountCriteria = contentCalendar.metadata.totalPieces >= 30; // Target for 12 weeks
    ...
    const complianceRate = ((linkingStrategy.metadata.totalOpportunities - linkingStrategy.metadata.policyViolations) / linkingStrategy.metadata.totalOpportunities) * 100;
    const complianceCriteria = complianceRate >= 90;
    criteriaResults.push(`Policy Compliance: ${complianceCriteria ? '✅' : '❌'} (${complianceRate.toFixed(0)}%/90%+ compliance)`);
    ...
    const validationReport = {
      ...
      success_criteria: {
        ...
        policy_compliance: { passed: complianceCriteria, actual: complianceRate, required: 90 },
    ...
    console.log(`Average opportunity score: ${contentCalendar.metadata.averageScore.toFixed(1)}`);
    console.log(`Policy compliance: ${complianceRate.toFixed(0)}%`);
...
module.exports = { runContentStrategyValidation };