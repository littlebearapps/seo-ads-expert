 ...
 * ✅ 2. CONFIRM daily budget is acceptable: $4/day
 * ✅ 3. CHECK geo targeting matches your requirements: AU, US
 ...
 * 
 * 📊 EXPECTED CHANGES:
 * - CREATE: 1 campaign (undefined - AU/US - 2025-09-03)
 * - CREATE: 2 ad groups
 * - CREATE: 2 keywords
...
function createCampaignStructure(allowHighBudget) {
  ...
  console.log('\n📊 Creating campaign: undefined - AU/US - 2025-09-03');
  console.log('💰 Daily Budget: $' + dailyBudget);
  
  // Create campaign
  const campaignBuilder = AdsApp.campaigns()
    .newCampaignBuilder()
    .withName('undefined - AU/US - 2025-09-03')
  ...
  geoTargets.push(SAFETY_CONFIG.GEO_CONSTANTS.US); // US
...
function createAdGroups(campaign) {
  console.log('\n🎯 Creating 2 ad groups...');
  
  const adGroups = [];
  // Ad Group 1: WebP PNG Conversion
  console.log('Creating ad group: WebP PNG Conversion');
  
  const adGroup0 = campaign.newAdGroupBuilder()
    .withName('WebP PNG Conversion')
    .withStatus('PAUSED')
  ...
  if (adGroup0.isSuccessful()) {
    ...
    console.log('✅ Ad group created: WebP PNG Conversion');
    
    // Apply label
    ag0.applyLabel(SAFETY_CONFIG.LABEL_PREFIX + '_' + DATE_STAMP);
    
    // Create keywords
    createKeywordsForAdGroup(ag0, [{"text":"webp to png","matchType":"EXACT","maxCpc":3}]);
    
    // Create responsive search ad
    createResponsiveSearchAd(ag0, {"headlines":["WebP PNG Conversion Chrome Extension","WebP PNG Conversion Tool","Free Browser Extension","Privacy-First File Conversion","Fast WebP PNG Conversion Tool"],"descriptions":["WebP PNG Conversion directly in your browser. Privacy-first, no uploads required. Fast and secure.","One-click WebP PNG Conversion conversion. Free Chrome extension with professional results."],"pinnedHeadlines":[{"text":"WebP PNG Conversion Chrome Extension","position":1}],"finalUrl":"https://example.com/webp?utm_source=google&utm_medium=cpc&utm_campaign={campaignid}&utm_content={adgroupid}&utm_term={keyword}"});
    
    adGroups.push(ag0);
  } else {
    console.error('❌ Failed to create ad group WebP PNG Conversion:', adGroup0.getErrors().join(', '));
  }
  // Ad Group 2: PDF to JPG Tools
  console.log('Creating ad group: PDF to JPG Tools');
  
  const adGroup1 = campaign.newAdGroupBuilder()
    .withName('PDF to JPG Tools')
    .withStatus('PAUSED')
    .withCpc(2.50) // Default max CPC
    .build();
    
  if (adGroup1.isSuccessful()) {
    const ag1 = adGroup1.getResult();
    console.log('✅ Ad group created: PDF to JPG Tools');
    
    // Apply label
    ag1.applyLabel(SAFETY_CONFIG.LABEL_PREFIX + '_' + DATE_STAMP);
    
    // Create keywords
    createKeywordsForAdGroup(ag1, [{"text":"pdf to jpg","matchType":"EXACT","maxCpc":3}]);
    
    // Create responsive search ad
    createResponsiveSearchAd(ag1, {"headlines":["PDF to JPG Tools Chrome Extension","PDF to JPG Tools Tool","Free Browser Extension","Privacy-First File Conversion","Fast PDF to JPG Tools Tool"],"descriptions":["PDF to JPG Tools directly in your browser. Privacy-first, no uploads required. Fast and secure.","One-click PDF to JPG Tools conversion. Free Chrome extension with professional results."],"pinnedHeadlines":[{"text":"PDF to JPG Tools Chrome Extension","position":1}],"finalUrl":"https://example.com/pdf?utm_source=google&utm_medium=cpc&utm_campaign={campaignid}&utm_content={adgroupid}&utm_term={keyword}"});
    
    adGroups.push(ag1);
  } else {
    console.error('❌ Failed to create ad group PDF to JPG Tools:', adGroup1.getErrors().join(', '));
  }
...
function previewCampaignChanges() {
  ...
  console.log('  Name: undefined - AU/US - 2025-09-03');
  console.log('  Budget: $4/day');
  console.log('  Status: PAUSED (requires manual activation)');
  console.log('  Markets: AU, US');
  console.log('  Device Targeting: Desktop only (mobile -100%)\n');
  
  console.log('🎯 AD GROUPS TO CREATE (' + 2 + '):');
  
  console.log('  1. WebP PNG Conversion');
  console.log('     Keywords: 1 (webp to png)');
  console.log('     Landing Page: https://example.com/webp');
  console.log('     Negatives: 11');
  console.log('  2. PDF to JPG Tools');
  console.log('     Keywords: 1 (pdf to jpg)');
  console.log('     Landing Page: https://example.com/pdf');
  console.log('     Negatives: 11');
  
  console.log('\n📊 SUMMARY:');
  console.log('  Total Keywords: 2');
  console.log('  Total Ads: 2');
  console.log('  Estimated Daily Spend: $4');
  ...
  console.log('  - Budget capped at $4/day');
